...
function resolvePinterestHref(rawUrl) {
  if (!rawUrl) return '';
  try {
...
function createPinterestEmbedSrcDoc(embedHtml) {
  return `<!doctype html>
<html>
<head>
<meta charset="utf-8">
<base target="_blank">
<meta http-equiv="Content-Security-Policy" content="default-src https: data:; script-src https://assets.pinterest.com https://*.pinterest.com; style-src 'unsafe-inline' https:; img-src https: data:; frame-src https://*.pinterest.com; object-src 'none'; form-action 'none'">
<style>html,body{margin:0;padding:0;background:transparent;}body{display:flex;justify-content:center;min-width:0;}</style>
</head>
<body>${embedHtml || ''}</body>
</html>`;
}
        ...
        {!loading && embedHtml && (
          <div className="mb-pinterest-viewport">
            <iframe
              ...
              srcDoc={createPinterestEmbedSrcDoc(embedHtml)}
            />
          </div>
        )}
        {!loading && !embedHtml && safeBoardUrl && (
...
export default function MoodBoard() {
  ...
  async function save() {
    ...
    const safePinterestUrl = payloadCategory === 'Pinterest' ? resolvePinterestHref(imageUrl) : '';
    if (payloadCategory === 'Pinterest' && (file || !safePinterestUrl)) {
      toast('Enter a valid Pinterest pin or board URL', 'error');
      return;
    }
    ...
    try {
      ...
      if (file) {
        ...
        payload = { imageUrl: safePinterestUrl || imageUrl, caption: form.caption, category: payloadCategory };
      }
