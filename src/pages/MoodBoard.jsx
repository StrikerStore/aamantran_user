...
function PinterestBoardCard({ eventId, boardUrl, caption, onDelete }) {
  ...
  const [embedState, setEmbedState] = useState({ requestKey: '', html: null });
  ...
  useEffect(() => {
    ...
    api.moodboard
      ...
      .then((r) => {
        if (cancelled) return;
        setEmbedState({ requestKey, html: r.html || null });
      })
      .catch(() => {
        if (!cancelled) setEmbedState({ requestKey, html: null });
      });
  ...
  const loading = Boolean(pinterestUrl && embedState.requestKey !== requestKey);
  const embedHtml = !loading && embedState.requestKey === requestKey ? embedState.html : null;
  ...
  return (
    <div className="mb-pinterest-card">
      ...
      <div className="mb-pinterest-body">
        ...
        {!loading && !embedHtml && pinterestUrl && (
          <div className="mb-pinterest-viewport mb-pinterest-fallback">
            <p>Pinterest preview is unavailable. Open it on Pinterest to view the board or pin.</p>
          </div>
        )}
        {!pinterestUrl && (
          <div className="mb-pinterest-viewport mb-pinterest-fallback mb-pinterest-invalid">
            <p>This saved Pinterest item has an invalid URL and was not loaded.</p>
          </div>
        )}
      </div>
...
export default function MoodBoard() {
  ...
  async function save() {
    ...
    const pinterestUrl = payloadCategory === 'Pinterest' ? normalizePinterestUrl(trimmedImageUrl) : null;
    if (payloadCategory === 'Pinterest') {
      if (file || !pinterestUrl) {
        toast('Enter a valid Pinterest board or pin URL', 'error');
        return;
      }
    }
