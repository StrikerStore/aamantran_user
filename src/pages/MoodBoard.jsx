...
function getTrustedPinterestUrl(value) {
  try {
    ...
    if (url.protocol !== 'https:' || (!isPinterestHost && !isShortHost)) return null;
    return url.href;
  } catch {
    return null;
  }
...
function pinterestFrameSrcDoc(innerHtml) {
  return `<!doctype html><html><head><base target="_blank"><style>html,body{margin:0;padding:0;background:transparent;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;}body{display:flex;justify-content:center;align-items:flex-start;min-height:180px;}iframe{max-width:100%!important;}</style></head><body>${innerHtml}</body></html>`;
}

function pinterestFallbackSrcDoc(url) {
  ...
  return pinterestFrameSrcDoc(
    `<a href="${safeUrl}" data-pin-do="${pinDo}"${attrs}></a><script async defer src="https://assets.pinterest.com/js/pinit.js"></script>`,
  );
}
...
function PinterestBoardCard({ eventId, boardUrl, caption, onDelete }) {
  const [embedResult, setEmbedResult] = useState({ key: null, html: null, loaded: false });
  const trustedBoardUrl = getTrustedPinterestUrl(boardUrl);
  const embedRequestKey = trustedBoardUrl ? `${eventId}:${trustedBoardUrl}` : null;
  const embedHtml = embedRequestKey && embedResult.key === embedRequestKey ? embedResult.html : null;
  const loading = Boolean(embedRequestKey && (embedResult.key !== embedRequestKey || !embedResult.loaded));
  const fallbackSrcDoc = trustedBoardUrl ? pinterestFallbackSrcDoc(trustedBoardUrl) : null;
  ...
  useEffect(() => {
    ...
    api.moodboard
      .pinterestOembed(eventId, trustedBoardUrl)
      .then((r) => {
        if (cancelled) return;
        setEmbedResult({ key: embedRequestKey, html: r.html || null, loaded: true });
      })
      .catch(() => {
        if (!cancelled) setEmbedResult({ key: embedRequestKey, html: null, loaded: true });
      });

    return () => {
      cancelled = true;
    };
  }, [eventId, trustedBoardUrl, embedRequestKey]);
      ...
      <div className="mb-pinterest-body">
        ...
        {!loading && !trustedBoardUrl && (
          <div className="mb-pinterest-viewport mb-pinterest-viewport--widget">
            <div className="mb-pinterest-widget-host">
              <p className="mb-pinterest-hint">This saved Pinterest URL is invalid and was not loaded.</p>
            </div>
          </div>
        )}
        {!loading && trustedBoardUrl && embedHtml && (
        ...
        {!loading && trustedBoardUrl && !embedHtml && fallbackSrcDoc && (
          <div className="mb-pinterest-viewport mb-pinterest-viewport--widget">
            <iframe
              className="mb-pinterest-embed-frame"
              title={caption ? `Pinterest preview: ${caption}` : 'Pinterest preview'}
...
export default function MoodBoard() {
  ...
  async function save() {
    ...
    let payloadImageUrl = trimmedImageUrl;
    if (payloadCategory === 'Pinterest') {
      if (file) {
        toast('Pinterest entries must use an HTTPS Pinterest board or pin URL', 'error');
        return;
      }
      const trustedPinterestUrl = getTrustedPinterestUrl(trimmedImageUrl);
      if (!trustedPinterestUrl) {
        toast('Enter a valid HTTPS Pinterest board or pin URL', 'error');
        return;
      }
      payloadImageUrl = trustedPinterestUrl;
    }
    ...
    try {
      ...
      if (file) {
        ...
        payload = { imageUrl: payloadImageUrl, caption: form.caption, category: payloadCategory };
      }
