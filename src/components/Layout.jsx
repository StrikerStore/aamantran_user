...
export function Layout() {
  ...
  useEffect(() => {
    function handler(e) {
      ...
      if (mobileSwitcherRef.current && !mobileSwitcherRef.current.contains(e.target)) setMobileSwitcherOpen(false);
    }
  ...
  return (
    <div className={`app-shell${railCollapsed ? ' rail' : ''}`}>
      ...
      <div className="main">
        ...
        <main className="page-content page-fade" id="main">
      ...
      <nav className="bottom-nav" aria-label="Main menu">
        <button type="button" className={`bottom-nav-item ${bottomActive.home ? 'active' : ''}`} onClick={() => { setMoreOpen(false); navigate('/dashboard'); }}>
        ...
        >
          <span className="bnav-fab"><Share2 className="bnav-icon" aria-hidden="true" /></span>
          <span>Share</span>
        </button>
        <button
          type="button"
          className={`bottom-nav-item ${bottomActive.guests ? 'active' : ''}`}
          aria-disabled={!eid || undefined}
